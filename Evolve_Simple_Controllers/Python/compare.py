...
def Set_Mods(r,mods1,mods2,mods3):

	fileName = '../Data/runData_'+str(r)+'.dat';
	f = open(fileName,'r');
	i = 0;
	for line in f.readlines():
		x=1;
	line = line.split();
	mods1.append(float(line[5]));
        mods2.append(float(line[6]));
        mods3.append(float(line[7]));
	f.close();

	return mods1,mods2,mods3;
...
for r in range(0,99+1):
	[mods1,mods2,mods3] = Set_Mods(r,mods1,mods2,mods3);

print sum(mods1)/len(mods1);
print sum(mods2)/len(mods2);
print sum(mods3)/len(mods3);
