...
ROBOT::ROBOT(int robotIndex) {

	char fileName[100];

	sprintf(fileName,"../RobotEvolvedBodies/Data/Robot_Matrix_%d_0.txt",robotIndex);

	std::ifstream infile(fileName);

	if ( infile.good() )

		robot = new MATRIX(fileName,NUM_BODY_PARTS,5);

	else {

		printf("ERROR: Robot body in %s not found.\n",fileName);
		exit(0);
	}
}
...
int  ROBOT::Claws_Are_Crossed(void) {
