...
extern int MAX_NON_DOMINATED_GENOMES;
...
void AFPO::Evolve(int randomSeed, int selectForAttractors, int useEvolvedBodies) {
	...
	for (currGeneration=0;currGeneration<NUM_GENERATIONS;currGeneration++) {
		...
		Prune_Overcrowded_Pareto_Front();
...
int  AFPO::Find_Most_Similar_Genome(void) {

	double minDifference = 1000000.0;
	int    targettedGenomeIndex = numNonDominated-1;

	for (int i=0;i<numNonDominated-1;i++)

		for (int j=i+1;j<numNonDominated;j++) {

			double difference = fabs( genomes[i]->fitness - genomes[j]->fitness );

			if ( difference < minDifference ) {

				minDifference = difference;
				targettedGenomeIndex = j;
			}
		}

	return targettedGenomeIndex;
}
...
void AFPO::Prune_Overcrowded_Pareto_Front(void) {

	while ( numNonDominated > MAX_NON_DOMINATED_GENOMES ) {

		int targettedGenomeIndex = Find_Most_Similar_Genome();

		genomes[targettedGenomeIndex]->dominated = true;

		GENOME *temp = genomes[numNonDominated-1];
		genomes[numNonDominated-1] = genomes[targettedGenomeIndex];
		genomes[targettedGenomeIndex] = temp;
		temp = NULL;
...
void AFPO::Sort(void) {
	...
	if ( genomes[POP_SIZE-1]->Is_Dominated() == false ) {

		genomes[POP_SIZE-1]->dominated = true;
		numNonDominated--;
	}
...
void AFPO::Sort_By_Modularity(void) {

        for (int c = 0 ; c < ( numWithMaximalGraspingAbility - 1 ); c++) {

                for (int d = 0 ; d < numWithMaximalGraspingAbility - c - 1; d++) {

                        if ( genomes[d]->modularity < genomes[d+1]->modularity ) {
